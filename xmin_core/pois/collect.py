...
def get_city_pois_categories(
    ...
    pois_cate_filenames = {}
    for category in tqdm(poi_categories, total=len(poi_categories), desc='Getting POIs per category'):
        log.info(f'Getting pois modes for {category.name}')
        most_pois_cate = ox.features.features_from_polygon(buffered_polygon, tags=category.value)
