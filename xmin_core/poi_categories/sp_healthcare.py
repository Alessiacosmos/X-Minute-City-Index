from xmin_core.poi_categories.base import POICatogories, Category, SubCategory


class DEHealthCareCategories(POICatogories):
    # special poi_category setting for Germany (DE)
    healthcare = Category(
        weight=0.1,
        subcategories=[
            SubCategory(
                name="primary_care",
                sub_weight=0.4,
                tag="(amenity=doctors or healthcare=doctor or amenity=clinic) and "
                "(healthcare:speciality=general or healthcare:speciality=internal)",
                benchmark=3,
            ),
            SubCategory(
                name="dentist",
                sub_weight=0.1,
                tag="amenity=dentist or healthcare=dentist",
                benchmark=1,
            ),
            SubCategory(
                name="pharmacy",
                sub_weight=0.3,
                tag="amenity=pharmacy or healthcare=pharmacy",
                benchmark=2,
            ),
            SubCategory(
                name="specialist",
                sub_weight=0.2,
                tag="healthcare:speciality=ophtalmology or healthcare:speciality=paediatrics or "
                "healthcare:speciality=gynaecology or healthcare:speciality=dermatology or "
                "healthcare:speciality=emergency",
                benchmark=3,
            ),
        ],
    )


class NLHealthCareCategories(POICatogories):
    # special poi_category setting for the Netherlands (NL)
    healthcare = Category(
        weight=0.1,
        subcategories=[
            SubCategory(
                name="primary_care",
                sub_weight=0.39,
                tag="(amenity=doctors or healthcare=doctor or amenity=clinic) and "
                "(healthcare:speciality=general or healthcare:speciality=internal)",
                benchmark=3,
            ),
            SubCategory(
                name="dentist",
                sub_weight=0.1,
                tag="amenity=dentist or healthcare=dentist",
                benchmark=1,
            ),
            SubCategory(
                name="pharmacy",
                sub_weight=0.5,
                tag="amenity=pharmacy or healthcare=pharmacy",
                benchmark=2,
            ),
            SubCategory(
                name="specialist",
                sub_weight=0.01,
                tag="healthcare:speciality=ophtalmology or healthcare:speciality=paediatrics or "
                "healthcare:speciality=gynaecology or healthcare:speciality=dermatology or "
                "healthcare:speciality=emergency",
                benchmark=3,
            ),
        ],
    )


class ESHealthCareCategories(POICatogories):
    # special poi_category setting for Spain (ES)
    healthcare = Category(
        weight=0.1,
        subcategories=[
            Category(
                group="primary_care",
                weight=0.49,
                subcategories=[
                    SubCategory(
                        name="primary_care_clinic",
                        sub_weight=1,
                        tag="amenity=clinic",
                        benchmark=1,
                    ),
                    SubCategory(
                        name="primary_care_other",
                        sub_weight=1,
                        tag="amenity!=clinic and "
                        "(amenity=doctors or healthcare=doctor) and "
                        "(healthcare:speciality=general or healthcare:speciality=internal)",
                        benchmark=3,
                    ),
                ],
            ),
            SubCategory(
                name="dentist",
                sub_weight=0.1,
                tag="amenity=dentist or healthcare=dentist",
                benchmark=1,
            ),
            SubCategory(
                name="pharmacy",
                sub_weight=0.4,
                tag="amenity=pharmacy or healthcare=pharmacy",
                benchmark=2,
            ),
            SubCategory(
                name="specialist",
                sub_weight=0.01,
                tag="healthcare:speciality=ophtalmology or healthcare:speciality=paediatrics or "
                "healthcare:speciality=gynaecology or healthcare:speciality=dermatology or "
                "healthcare:speciality=emergency",
                benchmark=3,
            ),
        ],
    )
